...
> **Важный нюанс:** это верно только для обычных (не помеченных `!important`) объявлений. Если объявление внутри слоя помечено `!important`, порядок приоритета слоёв **инвертируется** — побеждает `!important` из самого раннего слоя, а не из самого позднего. Кроме того, стили вне каких-либо слоёв (unlayered) обычно перевешивают все слои, но для `!important` всё наоборот: unlayered `!important` имеет наименьший приоритет среди всех `!important`-объявлений. Эта асимметрия — частый источник ошибок при миграции проектов на `@layer`, и о ней стоит помнить, прежде чем полагаться на интуицию «последний слой всегда побеждает».
...
4. **Отказ от `!important`** — слои делают его ненужным в большинстве сценариев
5. **Документация** — слои документируют архитектуру

> **Предупреждение:** если `!important` всё же используется внутри слоёв, помните, что его приоритет работает в обратном порядке относительно обычных объявлений — побеждает `!important` из самого раннего объявленного слоя. Например, `!important` внутри `@layer reset` перевесит `!important` внутри `@layer overrides`. Именно поэтому спецификация рекомендует использовать `!important` внутри слоёв крайне ограниченно — например, только в самом первом слое (`reset`), чтобы защитить критичные стили от случайного переопределения, а не как способ «пробить» более поздние слои.
...
**5. Избегайте `!important`, а если он всё же нужен — помните про инверсию порядка слоёв**
...
Если `!important` необходим — например, для защиты критичных стилей сторонней библиотеки или системного компонента, — размещайте его в самом раннем слое из объявленных. Порядок приоритета `!important` внутри слоёв обратный обычному: побеждает самый ранний слой, а не самый поздний. Использование `!important` в позднем слое (`overrides`) в расчёте на то, что он «перевесит всё остальное», — частая ошибка, которая приводит к противоположному результату.
