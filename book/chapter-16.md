...
Cross-document переходы всё ещё имеют неодинаковую поддержку браузеров: они уже реализованы в Chrome/Edge 126+ и Safari 18+, но Firefox at-rule `@view-transition` пока полностью игнорирует, и навигация в нём выполняется как обычная, мгновенная, без анимации — это безопасное поведение по умолчанию, не требующее отдельного кода.

```javascript
// Проверка поддержки same-document переходов
...
```css
/* ❌ Некорректно — "view-transition" не является CSS-свойством,
   и такая проверка через синтаксис "свойство: значение" не работает
   для проверки поддержки at-rule */
...
/* ✅ Корректно — проверка поддержки конкретного at-rule 
   через функцию at-rule() */
...
На практике отдельная `@supports`-обёртка для `@view-transition` обычно не обязательна: браузеры, которые не понимают неизвестный at-rule, попросту его игнорируют, и cross-document переход не запускается — сайт продолжает работать с обычной, мгновенной навигацией. Явная проверка через `at-rule()` полезна, когда нужно предоставить принципиально иной набор стилей или логику именно для браузеров без поддержки (а не просто положиться на автоматическую деградацию).
