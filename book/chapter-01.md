...
## 1.2 Почему CSS больше не является "языком стилей" *(добавлено уточнение)*
...
> **Статус поддержки:** CSS Anchor Positioning на момент выхода книги остаётся Limited availability — реализация в Firefox всё ещё неполная (в частности, есть открытые проблемы с привязкой к отдельным псевдо-элементам). В продакшене эту возможность стоит использовать вместе с прогрессивным улучшением, а не как единственный механизм позиционирования.
...
| Миксины | Custom Properties + `calc()`/`color-mix()`, частично — экспериментальный `@function` | Логика в рантайме там, где уже поддерживается; полноценная замена ещё не готова |
...
> **Важная оговорка:** нативная замена именно Sass-миксинов — правила `@mixin` и `@apply` из спецификации CSS Custom Functions and Mixins Module — пока не реализована ни в одном браузере. Того, что уже можно использовать сегодня (Custom Properties, `calc()`, вложенность, а в Chromium — экспериментальный `@function`), достаточно для многих сценариев, но полная замена миксинов Sass — вопрос будущего, а не текущего Baseline.
...
- Переиспользуемые миксины с аргументами — до тех пор, пока `@mixin`/`@apply` не получат поддержку браузеров
...
## 1.10 Baseline и новая модель совместимости *(переписано)*
...
**Что такое Baseline на самом деле:**

Baseline — это система из **двух ступеней зрелости**, которую определяет WebDX Community Group и публикуют MDN и web.dev:

* **Newly available** — функция реализована в актуальных стабильных версиях всех core-браузеров: Chrome, Edge, Firefox и Safari (включая мобильные версии). Это означает межбраузерную совместимость, но не гарантирует, что у всех пользователей уже обновлённый браузер.
* **Widely available** — с момента достижения Newly available прошло **30 месяцев**. К этому моменту функцию можно использовать почти без оглядки на поддержку — даже отстающие устройства успевают обновиться.

Функции, которые ещё не реализованы во всех core-браузерах, находятся в статусе **Limited availability** — именно так в этой книге помечаются, например, Anchor Positioning или Paint API из Houdini.

**Как это выглядит на практике:**

```text
Функция реализована в Chrome, Edge, Firefox, Safari
  ↓
...
Помимо статуса отдельной функции, ежегодно формируется срез уже накопленных изменений — «Baseline 2026» и подобные обозначения по годам, — но это скорее удобный ориентир для дайджестов, а не отдельный технический критерий: решающими остаются именно статусы Newly/Widely available у конкретной функции.

**Преимущества для разработчика:**
- Понятная двухступенчатая шкала вместо разрозненных таблиц совместимости
- Можно осознанно выбирать: использовать Newly available с осторожностью или ждать Widely available
- Обновления Baseline отслеживаются централизованно на MDN и web.dev
...
**Новая философия:** вместо борьбы с браузерами — осознанный выбор уровня зрелости функции: Limited, Newly или Widely available, — исходя из целевой аудитории проекта.
