...
## 14.7 Экспериментальные возможности *(добавлена конкретика о статусе поддержки)*
...
Эти технологии открывают новые способы описания геометрии компонентов — например, «сквирклы» (squircle) в стиле иконок iOS, скосы, вогнутые «зачерпнутые» и зазубренные углы, — однако пока не входят в Baseline и требуют использования `@supports` и безопасной деградации.

> **Статус поддержки:** на момент выхода книги `corner-shape` реализован только в Chromium-браузерах (Chrome и Edge, начиная с версии 139) — это чисто Limited availability возможность без объявленных сроков реализации в Firefox или Safari. Важная деталь для проектирования: в браузерах без поддержки свойство `corner-shape` просто игнорируется, а `border-radius` продолжает работать как обычно — то есть деградация происходит автоматически, без необходимости явного фолбэка через `@supports` для большинства простых случаев. Явная проверка через `@supports` нужна в первую очередь тогда, когда форма угла критична для восприятия компонента (например, в брендированных элементах), а не как техническая необходимость для предотвращения поломки вёрстки.

```css
/* Экспериментально — Chromium-only */
...
/* Безопасная деградация — работает везде, включая Chromium без @supports */
.card {
  border-radius: var(--radius-lg);
}
```
...
Экспериментальные возможности
  ↓
Использовать только с @supports для критичных по бренду случаев
  ↓
Полагаться на автоматическую деградацию border-radius в остальных
  ↓
