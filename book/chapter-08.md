...
> **Статус на момент написания книги:** технология достигла статуса **Baseline 2026** в январе 2026 года, когда её поддержка появилась в Firefox 147 без флагов (Chrome/Edge — с версии 125, Safari — начиная с 18.2). Это значит, что она уже готова к использованию в продакшне для широкой аудитории (покрытие ~91% глобального трафика), но проект с более ранними требованиями к поддержке браузеров (в частности, к Safari до версии 18.4) всё ещё нуждается в запасном варианте — подробности в разделе 8.5.
...
**Современное поведение (Chrome, Firefox, Safari 18.4+):** если элемент перешёл на fallback-позицию, он остаётся в ней, пока она не переполнится.

**Safari 18.2-18.3:** не поддерживает `@position-try` — базовое размещение работает, но авто-переворот при нехватке места отсутствует.
...
### Gotcha 1: `@position-try` в Safari 18.2-18.3

**Проблема:** Safari 18.2 и 18.3 поддерживают базовое размещение (`anchor-name`, `position-anchor`, `anchor()`), но не поддерживают `@position-try`.

**Решение:** всегда явно задавайте безопасную базовую позицию элемента вне блока `@position-try`:

```css
/* ✅ Безопасно для Safari 18.2+ */
.tooltip {
  position: absolute;
  position-anchor: --btn;
  
  /* Базовая позиция — работает везде */
  top: anchor(bottom);
  left: anchor(center);
  transform: translateX(-50%);
  margin-top: 6px;
  
  /* Safari 18.2-18.3 игнорирует, 18.4+ использует */
  position-try-fallbacks: flip-block;
}

/* ❌ Сломается в Safari 18.2-18.3 */
...
7. **Три главных gotcha:**
   - Safari 18.2-18.3 не поддерживает `@position-try` → пишите базовую позицию
   - `popover="hint"` — Chrome-only → используйте CSS `:hover`
   - Повторяющиеся компоненты → используйте `anchor-scope`

8. **Anchor Positioning — Baseline 2026** — покрытие ~91% глобального трафика
