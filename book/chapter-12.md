...
## 12.6 Адаптивные цветовые системы *(обновлён статус `contrast-color()`)*
...
> **Статус зрелости:** `contrast-color()` достиг статуса **Baseline Newly available в апреле 2026 года** — поддержку добавили все три core-браузера: Chrome 147, Firefox 146 и Safari 26 (обратите внимание на новую схему нумерации версий Safari, синхронизированную с годом ОС, — «Safari 26» здесь совершенно уместная и актуальная версия, а не опечатка). Это ощутимо более зрелый статус, чем «чистый эксперимент»: функцию уже можно применять в новых проектах без флагов. Тем не менее до статуса Widely available (то есть до истечения 30-месячного периода, ~октябрь 2028 года) для проектов с широкой поддержкой устаревших браузеров разумно сохранять `@supports`-фолбэк — просто по причине версий браузеров ниже указанных, а не из-за незрелости самой спецификации.
>
> Отдельная деталь для точности: Safari поддерживает нестандартное расширение синтаксиса — `contrast-color(var(--bg) max, white, black)`, где ключевое слово `max` явно требует алгоритма максимального контраста. Firefox и базовая спецификация такие ключевые слова алгоритма пока не поддерживают, так что полагаться на это расширение в кросс-браузерном коде не стоит.
...
8. **`contrast-color()`** — автоматический подбор контрастного текста (Baseline Newly available с апреля 2026 года; для устаревших браузеров всё ещё нужен `@supports`-фолбэк)
