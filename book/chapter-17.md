...
> **Статус на момент написания книги:** Scroll-driven Animations (`animation-timeline: scroll()`, `view()`, именованные таймлайны, `animation-range`) остаются в статусе **Limited availability** и формально не входят в Baseline. Chrome и Edge поддерживают спецификацию давно и стабильно — с версии 115 (июль 2023 года). Safari добавил поддержку в версии 26 (сентябрь 2025 года). Firefox — единственное исключение: в стабильной версии (152, июнь 2026 года) функциональность всё ещё скрыта за флагом `layout.css.scroll-driven-animations.enabled`, хотя по умолчанию включена в Nightly-сборках и входит в число приоритетов Interop 2026, то есть Mozilla явно намерена её выпустить. Совокупное глобальное покрытие браузеров на середину 2026 года — около 82–84%. Для продакшена рекомендуется относиться к Scroll-driven Animations как к прогрессивному улучшению.
...
### Progressive Enhancement для браузеров без поддержки

Поскольку Firefox в стабильной версии пока не поддерживает Scroll-driven Animations, важно проектировать компонент так, чтобы отсутствие анимации не ломало интерфейс:

```css
/* Базовое состояние — контент виден всегда, даже без поддержки анимации */
.card {
  opacity: 1;
  transform: none;
}

/* Прогрессивное улучшение — применяется только при поддержке */
@supports (animation-timeline: view()) {
  .card {
    ...
    /* Начальное состояние анимации только внутри @supports,
       чтобы в Firefox контент не остался невидимым */
    opacity: 0;
...
Ключевое архитектурное правило: **начальные (нулевые) значения анимируемых свойств должны задаваться только внутри `@supports`-блока**, а не в базовых стилях компонента. Иначе в браузере без поддержки (сегодня — Firefox stable) элемент так и останется невидимым или смещённым, поскольку анимация, которая должна была вернуть его в нормальное состояние, никогда не запустится.
...
**7. Оборачивайте в `@supports` и держите базовое состояние видимым**

```css
/* ✅ Рекомендуется — контент виден по умолчанию */
.card { opacity: 1; }
...
Это особенно важно до тех пор, пока Firefox stable не выпустит поддержку без флага — на момент написания книги это единственный из core-браузеров, ещё не завершивший переход.
---
